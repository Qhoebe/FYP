...
class PretrainedSSD(nn.Module):
    def __init__(self, threshold=0.5): 
        super().__init__()
        self.model = models.detection.ssdlite320_mobilenet_v3_large(weights='DEFAULT')
...
def pretrained_evaluation(is_blurred = False):
    ...
    model = PretrainedSSD(threshold=0.5).to(DEVICE)
    ...
    if is_blurred:
        with open(f'{ROOT_DIR}/results/pretrained_ssd_results_on_blurred_images.json', "w") as f:
            json.dump(results, f, indent=4)
    else: 
        with open(f'{ROOT_DIR}/results/pretrained_ssd_results.json', "w") as f:
            json.dump(results, f, indent=4)
